...
- Leveraged AWS Glue (PySpark) and Apache Airflow for feature engineering and persistent storage in AWS ElastiCache (Redis OSS).
- Engineered a high-throughput Python model-serving layer containerized via Docker on AWS EKS/Fargate, maintaining sub-second latency for synchronized feature retrieval and real-time model querying.
